...
# print ''
    ...
    def getVarLen(self):
        """Return the var of all br.len's."""
        # About 2 msec for a tree with 1000 leaves.
        br_list=[]
        for n in self.iterNodesNoRoot():
            br_list.append(n.br.len)
        if br_list==[]:
            return 0
        else:
            return numpy.var(numpy.array(br_list))

    def getPartLen(self,node):
        """Return the sum of all br.len's."""
        # About 2 msec for a tree with 1000 leaves.
        total1 = 0.0
        total2 = 0.0
        for n in self.iterNodesNoRoot():
            if node.isAncestorOf(n) or node==n:
                total1 += n.br.len
            else:
                total2 += n.br.len
        return (total1,total2)

    def getPartMaxLen(self,node,node2=None):
        """Return the sum of all br.len's."""
        # About 2 msec for a tree with 1000 leaves.
        max1 = 0.0
        max2 = 0.0
        for n in self.iterNodesNoRoot():
            if node.isAncestorOf(n) or node==n:
                ...
                if node2 is not None:
                    if node2.isAncestorOf(n) or node==n:
                        continue
                if n.br.len > max2:
                    max2 = n.br.len
        return (max1,max2)

    def getPartVarLen(self,node,node2=None):
        """Return the sum of all br.len's."""
        # About 2 msec for a tree with 1000 leaves.
        br_list1=[]
        br_list2=[]
        for n in self.iterNodesNoRoot():
            if node.isAncestorOf(n) or node==n:
                br_list1.append(n.br.len)
            else:
                br_list2.append(n.br.len)
        if br_list1==[]:
            res1=0
        else:
            res1=numpy.var(numpy.array(br_list1))
        if br_list2==[]:
            res2=0
        else:
            res2=numpy.var(numpy.array(br_list1))
        return (res1,res2,numpy.array(br_list1),numpy.array(br_list2))
...
#                        x = x | i
    ...
    def write(self):
        ...
        return self.writeNewick(sys.stdout)
