...
class TreePicture(object):
    ...
    def _setPos(self, autoIncreaseWidth=True):
        gm = ['TreePicture._setPos()']
        ...
        if self.maxX < 1.e-100:
            gm.append("maxX (%f, %g) is too small.  Can't draw it." %
                      (self.maxX, self.maxX))
            raise P4Error(gm)
        # Find maxY
        self.maxY = 0.0
        ...
        if self.maxY < 1.e-100:
            gm.append(
                "maxY (%f, %g) is too small.  How did that happen?" % (self.maxY, self.maxY))
            raise P4Error(gm)
        ...
        if self.xScale == None:
            ...
            if self.tree.root.name:
                ...
                thisPointsPerLetter = (
                    float(self.labelTextSize) / float(self.textSize)) * self.pointsPerLetter
                self.rootNameContribution = (
                    rootNameLength * thisPointsPerLetter) + self.nameOffset
            else:
                self.rootNameContribution = 0.0
            # print "self.rootNameContribution = %s" %
            # self.rootNameContribution
            ...
            for i in nameLengths:
                theSum = self.rootNameContribution + \
                    (i[1] * self.pointsPerLetter) + self.nameOffset
                if theSum >= self.width - (2.0 * self.xOrigin):
                    if autoIncreaseWidth:
                        self.width = theSum + (2.0 * self.xOrigin) + 1
                    else:
                        gm.append(
                            "The picture is not wide enough to fit the node names.")
    ...
    def vectorString(self):
        ...
        if self.svg:
            ...
            stringList.append(
                '<style type="text/css"><![CDATA[line {stroke:blue; stroke-width:5; stroke-linecap:round;}]]></style>\n')
            stringList.append('</defs>\n')
            stringList.append(
        ...
        for n in self.tree.iterNodesNoRoot():
            if self.svg:
                ...
                stringList.append('<line x1="%.0f" y1="%.0f" x2="%.0f" y2="%.0f"/>\n' % (
                    ...
                    (n.parent.xPos * self.xScale), self.yOrigin -
                    (n.yPos * self.yScale),
        ...
        for n in self.tree.iterNodes():
            if not n.isLeaf:
                ...
                if self.svg:
                    stringList.append('<line x1="%.0f" y1="%.0f" x2="%.0f" y2="%.0f"/>\n' % (
                        self.rootNameContribution + self.xOrigin +
                        (n.xPos * self.xScale), self.yOrigin -
            ...
            if n.name:
                if self.svg:
                    stringList.append('<text x="%.0f" y="%.0f">%s</text>\n' % (
                        self.rootNameContribution + self.xOrigin +
                        (n.xPos * self.xScale) + self.nameOffset,
        ...
        if self.tree.root.name:
            ...
            if n.isLeaf:
                if self.svg:
                    ...
                    # we want textSize here, but that is currently what it is,
                    # so no need to specify
                    stringList.append(
                        '%% Terminal root; should already be textSize, ie %s scalefont.\n' % self.textSize)
            else:
                if self.svg:
                    stringList.append('<text x="%.0f" y="%.0f" text-anchor="end">%s</text>\n' % (
            ...
            if self.svg:
                ...
                stringList.append(
                    '(%s) stringwidth pop -1.0 mul 0 rmoveto\n' % n.name)
        ...
        if not self.svg:
            ...
            if self.putInternalNodeNamesOnBranches:
                stringList.append(
                    '% (Here, internal node names are placed on branches).\n')
            else:
                stringList.append(
                    '% (Here, internal node names are placed on nodes).\n')
            stringList.append(
                '/Times-Roman findfont %i scalefont setfont\n' % self.labelTextSize)
        for n in self.tree.iterNodesNoRoot():
        ...
        if self.svg:
            stringList.append('</g>\n')
            stringList.append(
                '<use xlink:href="#tree1" transform="scale(1.0)"/>\n')
    ...
    def textString(self, returnAsList=False):
            ...
            if self.partNum > -1:
                ...
                if self.textDrawModelThing == var.TEXTDRAW_COMP:
                    if n.parts[self.partNum].compNum >= 0 and tp.comps[n.parts[self.partNum].compNum].symbol:
                        theSymbol = tp.comps[
                            n.parts[self.partNum].compNum].symbol
                    ...
                    if n.br.parts[self.partNum].rMatrixNum >= 0 and tp.rMatrices[n.br.parts[self.partNum].rMatrixNum].symbol:
                        theSymbol = tp.rMatrices[
                            n.br.parts[self.partNum].rMatrixNum].symbol
                    ...
                    if n.br.parts[self.partNum].gdasrvNum >= 0 and tp.gdasrvs[n.br.parts[self.partNum].gdasrvNum].symbol:
                        theSymbol = tp.gdasrvs[
                            n.br.parts[self.partNum].gdasrvNum].symbol
                else:
                    #raise P4Error("TreePicture.textString().  No good textDrawModelThing.  Fix me")
                    pass

            start = int(
                round(self.rootNameContribution + self.xOrigin + (n.parent.xPos * self.xScale)))
            end = int(
                round(self.rootNameContribution + self.xOrigin + (n.xPos * self.xScale)))
            # print "start = %s, end = %s, width = %s" % (start, end,
            # self.width)
            for i in range(start + 1, end):
                stringList[n.yPos][i] = theSymbol
            stringList[n.yPos][start] = '+'
            # When the root is a single monofurcating stem, replace the '+'
            # with theSymbol
        ...
        for n in self.tree.iterInternalsNoRoot():
            ...
            start = int(
                round(self.rootNameContribution + self.xOrigin + (n.xPos * self.xScale)))
            if self.textShowNodeNums:
                # We want to over-write the vertical line with the node number
                pass
            else:
        ...
        if self.partNum > -1:
            ...
            if self.textDrawModelThing == var.TEXTDRAW_COMP:
                stringList.append(
                    'The tree above shows part %s comps' % self.partNum)
                if self.tree.model.parts[self.partNum].nComps:
                    for i in range(self.tree.model.parts[self.partNum].nComps):
                        stringList.append(
                            '    %-3i %s' % (i, self.tree.model.parts[self.partNum].comps[i].symbol))
                    rt = self.tree.root
                    ...
                    stringList.append(
                        '    No comps defined for part %s.' % self.partNum)
            elif self.textDrawModelThing == var.TEXTDRAW_RMATRIX:
                stringList.append(
                    'The tree above shows part %s rMatrices' % self.partNum)
                if self.tree.model.parts[self.partNum].nRMatrices:
                    for i in range(self.tree.model.parts[self.partNum].nRMatrices):
                        stringList.append(
                            '    %-3i %s' % (i, self.tree.model.parts[self.partNum].rMatrices[i].symbol))
                else:
                    stringList.append(
                        '    No rMatrices defined for part %s.' % self.partNum)
            elif self.textDrawModelThing == var.TEXTDRAW_GDASRV:
                stringList.append(
                    'The tree above shows part %s gdasrvs' % self.partNum)
                if self.tree.model.parts[self.partNum].nGdasrvs:
                    for i in range(self.tree.model.parts[self.partNum].nGdasrvs):
                        stringList.append(
                            '    %-3i %s' % (i, self.tree.model.parts[self.partNum].gdasrvs[i].symbol))
                else:
                    stringList.append(
                        "    No gdasrv's defined for part %s." % self.partNum)
