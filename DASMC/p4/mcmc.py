...
class McmcTunings(object):
    ...
    def __init__(self, nParts):
        ...
        self.default['allBrLens'] = 2.0 * math.log(2.0)  # 1.386
        ...
        # self.default['esprLambda'] = 2.0 * math.log(1.6)
        # self.default['esprPExt'] = 0.8
...
class McmcProposalProbs(dict):
    ...
    def __init__(self):
        ...
        object.__setattr__(self, 'eSPR', 1.0)
    ...
    *checkPointInterval*.  So in this case 1000000 is ok, but 900000 is
    ...
    def _makeProposals(self):
        ...
        if self.prob.eSPR:
            ...
            p.brLenPriorType = self._tunings.default['brLenPriorType']
            p.brLenPriorLambda = self._tunings.default['brLenPriorLambda']
            p.weight = self.prob.eSPR * \
                (len(self.tree.nodes) - 1) * fudgeFactor['eSPR']
            self.props.proposals.append(p)
        ...
        for p in self.props.proposals:
            if p.name in ['local', 'eTBR', 'eSPR','NNI', 'polytomy']:
    ...
    def writeProposalAcceptances(self):
        ...
        # do the same for eSPR
        doTopol = 0
        p = self.props.topologyProposalsDict.get('eSPR')
        if p:
            for tNum in range(self.nChains):
                if p.nTopologyChangeAttempts[tNum]:
                    doTopol = 1
                    break
            if doTopol:
                print("'eSPR' proposal-- attempted topology changes")
                print(
                    "%s tempNum   nProps nAccepts percent nTopolChangeAttempts nTopolChanges percent" % spacer)
                for tNum in range(self.nChains):
                    print("%s" % spacer, end=' ')
                    print("%4i " % tNum, end=' ')
                    print("%9i" % p.nProposals[tNum], end=' ')
                    ...
                    print("%20i" % p.nTopologyChangeAttempts[tNum], end=' ')
                    print("%13i" % p.nTopologyChanges[tNum], end=' ')
                    print("  %5.1f" % (100.0 * float(p.nTopologyChanges[tNum]) / float(
                        p.nTopologyChangeAttempts[tNum])))
            else:
                print("%sFor the 'eSPR' proposals, there were no attempted" % spacer)
                print("%stopology changes in any of the chains." % spacer)

        # do the same for NNI
        doTopol = 0
        p = self.props.topologyProposalsDict.get('NNI')
        if p:
            for tNum in range(self.nChains):
                if p.nTopologyChangeAttempts[tNum]:
                    doTopol = 1
                    break
            if doTopol:
                print("'NNI' proposal-- attempted topology changes")
                print(
                    "%s tempNum   nProps nAccepts percent nTopolChangeAttempts nTopolChanges percent" % spacer)
                for tNum in range(self.nChains):
                    print("%s" % spacer, end=' ')
                ...
                print("%sFor the 'NNI' proposals, there were no attempted" % spacer)
                print("%stopology changes in any of the chains." % spacer)
        ...
        if p:
            if hasattr(p, 'nAborts'):
                if p.nAborts[0]:
                    print("The 'eSPR' proposal had %i aborts.  (Not counted in nProps above.)" % p.nAborts[0])
                    assert self.constraints
                    print("(Aborts due to violated constraints)")
                else:
                    if self.constraints:
                        print("The 'eSPR' proposal had no aborts (due to violated constraints).")

        p = self.props.topologyProposalsDict.get('NNI')
        if p:
            if hasattr(p, 'nAborts'):
                if p.nAborts[0]:
                    print("The 'NNI' proposal had %i aborts.  (Not counted in nProps above.)" % p.nAborts[0])
                    assert self.constraints
                    print("(Aborts due to violated constraints)")
                else:
                    if self.constraints:
                        print("The 'NNI' proposal had no aborts (due to violated constraints).")
    ...
    def run(self, nGensToDo, verbose=True, equiProbableProposals=False, writeSamples=True,DLmodel=None,SingleSite=False):
        ...
        abortableProposals = ['local', 'polytomy', 'compLocation', 'eTBR', 'eSPR', 'NNI',
