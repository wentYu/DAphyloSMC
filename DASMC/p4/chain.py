...
class Chain(object):
    ...
    def propose(self, theProposal):
        ...
        ##########################
            ...
            self.proposeAllBrLens_Modified(theProposal)
            self.propTree.setCStuff()
            pf.p4_setPrams(self.propTree.cTree, -1)
        ...
        elif theProposal.name == 'eSPR':
            # self.proposeESPR_Blaise(theProposal)
            features=self.proposeESPR_Blaise_FromETBR(theProposal,extract_features=True)[1]
            # self.proposeESPR(theProposal)
            if theProposal.doAbort:
                pass
            else:
                if not self.propTree.preAndPostOrderAreValid:
        ...
        elif theProposal.name == 'NNI':
            # self.proposeESPR_Blaise(theProposal)
            features=self.proposeNNI_Blaise(theProposal,extract_features=True)[1]
            # self.proposeESPR(theProposal)
            if theProposal.doAbort:
                pass
            else:
        ...
        # them.
        if 1:
            if theProposal.name in ['local', 'brLen', 'eTBR', 'eSPR', 'polytomy']:
    ...
    def proposeSp(self, theProposal):
        ...
        if theProposal.name == 'comp':
            ...
            pf.p4_partLogLike(self.propTree.cTree,self.propTree.data.parts[theProposal.pNum].cPart,theProposal.pNum, 0)
        ...
        #     #self.propTree.root.flag = 0
            ...
            if self.propTree.root.getNChildren() == 2:
                ...
                # self.propTree.makeSplitKeys()
                # print("rooter br is %s, rooter.nodeNum=%i" % (rooter.br, rooter.nodeNum))
                # print("Starting eSPR proposal ...")
                # self.proposeESPR_Blaise(theProposal)
                # self.proposeESPR_Blaise_FromETBR(theProposal)
                features = self.proposeESPR_Blaise_FromETBR(theProposal, extract_features=True)[1]
                # print("  ... finished eSPR proposal")
            ...
            if self.mcmc.constraints and theProposal.topologyChanged:
                ...
                ret = self.mcmc.constraints.areConsistentWithTree(self.propTree)
                if not ret:
                    # print("proposeSp() after eSPR, doAbort set due to violated constraints")
                    theProposal.doAbort = True
                    return 0.0
                if self.mcmc.constraints.rooting:
                    ret = self.mcmc.constraints.areConsistentWithTreeRoot(self.propTree)
                    if not ret:
                        # print("proposeSp() after eSPR, doAbort set due to violated constraint for root")
                        theProposal.doAbort = True
                        return 0.0

            # Bugfix, moved from proposeESPR_Blaise() to here, *after* checking for violated constraints.
            if theProposal.topologyChanged:
                for pNum in range(self.propTree.model.nParts):
                    # print("\n")
                    # print(self.propTree.model.parts[pNum].bQETneedsReset)
    ...
    def gen(self, aProposal):
        ...
        if 0:
            ...
            if aProposal.name in ['local', 'brLen', 'eTBR','eSPR', 'polytomy', 'allBrLens']:
                nnBrLenChanged = []
                nnFlags = []
                for n in self.propTree.iterNodes():
                    if n.br:
        ...
        if aProposal.doAbort:
            if aProposal.name in ['local', 'eTBR','eSPR']:
                ...
                while 1:
                    ...
                    if var.doMcmcSp:  # the speedy version
                        ...
                        pRet = self.proposeSp(aProposal)[0]
                    else:
                        pRet = self.propose(aProposal)
        ...
        elif aProposal.name in ['local', 'eTBR','eSPR','NNI', 'root3', 'root3n', 'root2',
    ...
    def verifyIdentityOfTwoTreesInChain(self, doSplitKeys=False):
        ...
        # cStuff.  This does model prams, tree and node stuff.
        # print("about to pf.p4_verifyIdentityOfTwoTrees({} {})".format(self.curTree.cTree, self.propTree.cTree))
        # ret = pf.p4_verifyIdentityOfTwoTrees(
    ...
    def proposeAllBrLens_Modified(self, theProposal):
        gm = ['Chain.proposeAllBrLens']
        # Choose a node.
        nodesNoRoot = [n for n in self.propTree.iterNodesNoRoot()]
        # print(self.logPriorRatio)
        self.logPriorRatio=0
        self.logProposalRatio=0
        total_old=0
        total_new=0
        for theNode in nodesNoRoot:
        # theNode = random.choice(nodesNoRoot)
        #theNode = self.propTree.nodes[1]
            oldBrLen = theNode.br.len
            ...
            if 1:  # "Multiplier" proposal
                ...
                if theProposal.brLenPriorType == 'exponential':
                    ...
                    total_new+=newBrLen
                # if theProposal.brLenPriorType == 'exponential':
                #     self.logPriorRatio += theProposal.brLenPriorLambda * (oldBrLen - newBrLen)
            ...
            if var.doMcmcSp:
                theNode.br.lenChanged = True
        # print(theProposal.brLenPriorLambda,total_old,total_new)
        self.logPriorRatio = theProposal.brLenPriorLambda * (total_old - total_new)
    ...
    def proposeETBR_Blaise_Modified(self, theProposal):
        """Adapted and modified from the Evans and Sullivan version
        ...
        etbrLambda = theProposal.etbrLambda
        etbrPExt_X = theProposal.etbrPExt
        etbrPExt_Y = 0
        ...
        if x0Uncon and r0 is not x1:  # has there been a rearrangement on the X-side?
            if y0Uncon:
                if not r0Uncon:
                    # x-side constrained, y-side unconstrained
                    lnProp += math.log(1.0 - etbrPExt_X)
            else:  # so y-side is constrained
                if r0Uncon:
                    # x-side unconstrained, y-side constrained
                    lnProp += math.log(1.0 / (1.0 - etbrPExt_X))
        ...
        if y0Uncon and s0 is not y1:  # has there been a rearrangement on the Y-side?
            if x0Uncon:
                if not s0Uncon:
                    # y-side constrained, x-side unconstrained
                    lnProp += math.log(1.0 - etbrPExt_Y)
            else:  # so y-side is constrained
                if s0Uncon:
                    # y-side unconstrained, x-side constrained
                    lnProp += math.log(1.0 / (1.0 - etbrPExt_Y))
    ...
    def proposeESPR_Blaise(self, theProposal, is_NNI=0,extract_features=False):
        ...
        if x0Uncon:
            ...
            if r0 == x1:
                ...
                try:
                    ...
                    features['Prune_brLen'] = 0
                try:
                    features['Regraft_brLen'] = r0.br.len
                except Exception as e:
                    features['Regraft_brLen'] = 0
                features['Prune2Regraft_nodeNum'] = 0
                features['Prune2Regraft_brLen'] = 0
                features['Subtree1_leafNum'] = len(list(y0.iterLeaves()))
                features['Subtree2_leafNum'] = pTree.data.nTax-len(list(y0.iterLeaves()))
                features['Subtree3_leafNum'] = len(list(r0.iterLeaves()))
                features['Subtree4_leafNum'] = features['Subtree2_leafNum']-len(list(r0.iterLeaves()))

                features['Subtree1_total_brLens'], features['Subtree2_total_brLens'] = pTree.getPartLen(y0)
                features['Subtree3_total_brLens']= pTree.getPartLen(r0)[0]
                features['Subtree4_total_brLens']= features['Subtree2_total_brLens']-features['Subtree3_total_brLens']

                features['Subtree1_max_brLen'], features['Subtree2_max_brLen'] = pTree.getPartMaxLen(y0)
                features['Subtree3_max_brLen'], features['Subtree4_max_brLen'] = pTree.getPartMaxLen(r0,y0)
                ...
                # pTree.draw()
                # print(myChoice.nodeNum, y0.nodeNum,x0.nodeNum, x1.nodeNum, r0.nodeNum, r1.nodeNum)
                ...
                len_list=[]
                y0_=y0
                while y0_.parent!=None:
                    if y0_.isAncestorOf(r0) or y0_==r0:
                        r0_=r0
                        while y0_!=r0_:
                            path+=1
                            r0_=r0_.parent
                ...
                features['Prune2Regraft_nodeNum'] = path
                features['Prune2Regraft_brLen'] = total
                features['Subtree1_leafNum'] = len(list(y0.iterLeaves()))
                features['Subtree2_leafNum'] = pTree.data.nTax-len(list(y0.iterLeaves()))
                features['Subtree3_leafNum'] = len(list(r0.iterLeaves()))
                features['Subtree4_leafNum'] = features['Subtree2_leafNum']-len(list(r0.iterLeaves()))
                ...
                features['Subtree1_max_brLen'], features['Subtree2_max_brLen'] = pTree.getPartMaxLen(y0)
                features['Subtree3_max_brLen'], features['Subtree4_max_brLen'] = pTree.getPartMaxLen(r0,y0)
                # features['Reform_brLen'] = y0.br.len+r0.br.len
        ...
        if 1:
            ...
            if x0Uncon:
                ...
                if features is not None:
                    features['Reshape_brLens'] = eA.len + eX.len
        ...
        for n in pTree.iterNodesNoRoot():
            ...
            if n.br.oldNode != n:
                ...
                # n.br.oldNode.nodeNum)
                n.br.lenChanged = 1
        # if pTree==self.propTree:
        #     print('test:the same')
        if extract_features:
            return (pTree,features)
        return pTree

    def proposeESPR_Blaise_Backup(self, theProposal):
        """SPR version of proposeETBR_Blaise
    ...
    def proposeESPR_Blaise_FromETBR(self,theProposal ,is_NNI=0, extract_features=False ):
        """Adapted and modified from the Evans and Sullivan version
        ...
        if x0Degree == 1:
            x0Uncon = False  # a leaf
            # feature_names=['Move_mode','Total_brLens','Max_brLen','Var_brLen','Prune_brLen','Regraft_brLen','Prune_regraft_brLen_ratio',
            #                'Prune2Regraft_total_leafNum','Prune2Regraft_total_brLens','Prune2Regraft_total_maxBrLen','Prune2Regraft_total_varBrLen',
            #                'Prune2Regraft_nodeNum','Prune2Regraft_brLens','Prune2Regraft_maxBrLen','Prune2Regraft_varBrLen','Subtree1_leafNum',
            #                'Subtree2_leafNum','Subtree3_leafNum','Subtree4_leafNum','Subtree1_total_brLens','Subtree2_total_brLens',
            #                'Subtree3_total_brLens','Subtree4_total_brLens','Subtree1/3_brLens_ratio','Subtree1_max_brLen',
            #                'Subtree2_max_brLen','Subtree3_max_brLen','Subtree4_max_brLen','Subtree1_var_brLen','Subtree3_var_brLen',
            #                'Subtree3_var_brLen','Subtree4_var_brLen','Large2LongRisk',
            #                'A_Total_brLens', 'A_Max_brLen', 'A_Var_brLen', 'A_Prune_brLen', 'A_Regraft_brLen',
            #                'A_Prune_regraft_brLen_ratio',
            #                'A_Prune2Regraft_total_leafNum', 'A_Prune2Regraft_total_brLens', 'A_Prune2Regraft_total_maxBrLen',
            #                'A_Prune2Regraft_total_varBrLen',
            #                'A_Prune2Regraft_nodeNum', 'A_Prune2Regraft_brLens', 'A_Prune2Regraft_maxBrLen',
            #                'A_Prune2Regraft_varBrLen', 'A_Subtree1_leafNum',
            #                'A_Subtree2_leafNum', 'A_Subtree3_leafNum', 'A_Subtree4_leafNum', 'A_Subtree1_total_brLens',
            #                'A_Subtree2_total_brLens',
            #                'A_Subtree3_total_brLens', 'A_Subtree4_total_brLens', 'A_Subtree1/3_brLens_ratio',
            #                'A_Subtree1_max_brLen',
            #                'A_Subtree2_max_brLen', 'A_Subtree3_max_brLen', 'A_Subtree4_max_brLen', 'A_Subtree1_var_brLen',
            #                'A_Subtree3_var_brLen',
            #                'A_Subtree3_var_brLen', 'A_Subtree4_var_brLen', 'A_Large2LongRisk',
            #                'Reshape_brLens','Reshape_mul_brLens']
            features = {}
            ...
            features['Subtree1_var_brLens'], features['Subtree2_var_brLens'],_,br_list2 = pTree.getPartVarLen(y0)
            features['Subtree3_var_brLens'],br_list1 = 0,[]


            unique_vals, counts = numpy.unique(br_list1, return_counts=True)
            ...
            # features['A_Total_brLens'] = pTree.getLenMirror(c=0)
            # features['A_Max_brLen'] = pTree.getMaxLen()
            # features['A_Var_brLen'] = pTree.getVarLen()
            #
            # try:
            #     features['A_Prune_brLen'] = y0.br.len
            # except Exception as e:
            #     features['A_Prune_brLen'] = 0
            # features['A_Regraft_brLen'] = 0
            # features['A_Prune_regraft_brLen_ratio'] = -1
            #
            # features['A_Prune2Regraft_total_leafNum'] = 0
            # features['A_Prune2Regraft_total_brLens'] = 0
            # features['A_Prune2Regraft_total_MaxbrLen'] = 0
            # features['A_Prune2Regraft_total_VarbrLen'] = 0
            #
            # features['A_Prune2Regraft_nodeNum'] = 0
            # features['A_Prune2Regraft_brLen'] = 0
            # features['A_Prune2Regraft_Max_brLen']=0
            # features['A_Prune2Regraft_Var_brLen'] = 0
            #
            # features['A_Subtree1_leafNum'] = len(list(y0.iterLeaves()))
            # features['A_Subtree2_leafNum'] = pTree.data.nTax - len(list(y0.iterLeaves()))
            # features['A_Subtree3_leafNum'] = 0
            # features['A_Subtree4_leafNum'] = pTree.data.nTax - len(list(y0.iterLeaves()))
            # features['A_Subtree1_total_brLens'], features['A_Subtree2_total_brLens'] = pTree.getPartLen(y0)
            # features['A_Subtree3_total_brLens'] = 0
            # features['A_Subtree4_total_brLens'] = features['A_Subtree2_total_brLens'] - features[
            #     'A_Subtree3_total_brLens']
            # features['A_Subtree1/3_brLens_ratio'] = -1
            #
            # features['A_Subtree1_max_brLen'], features['A_Subtree2_max_brLen'] = pTree.getPartMaxLen(y0)
            # features['A_Subtree3_max_brLen'], features['A_Subtree4_max_brLen'] =0, features['A_Subtree2_max_brLen']
            #
            # features['A_Subtree1_var_brLens'], features['A_Subtree2_var_brLens'],_,br_list2 = pTree.getPartVarLen(y0)
            # features['A_Subtree3_var_brLens'],br_list1 = 0,[]
            #
            # unique_vals, counts = numpy.unique(br_list1, return_counts=True)
            # freq_dict = dict(zip(unique_vals, counts))
            # mask = numpy.ones(len(br_list2), dtype=bool)
            ...
            # else:
            #     res4 = numpy.var(br_list2[mask])
            # features['A_Subtree4_var_brLens'] = res4
            #
            # features['A_Large2LongRisk']=features['A_Subtree1_leafNum']*features['A_Regraft_brLen']
        ...
        if x0Uncon:
            ...
            if r0 == x1:
                ...
                try:
                    ...
                    features['Regraft_brLen'] = 0
                if features['Regraft_brLen'] ==0:
                    features['Prune_regraft_brLen_ratio'] = -1
                else:
                    try:
                        features['Prune_regraft_brLen_ratio'] = features['Prune_brLen']/features['Regraft_brLen']
                    except Exception as e:
                        features['Prune_regraft_brLen_ratio'] = -1
                features['Prune2Regraft_leafNum'] = 0
                features['Prune2Regraft_total_brLens'] = 0
                ...
                features['Subtree1_total_brLens'], features['Subtree2_total_brLens'] = pTree.getPartLen(y0)
                features['Subtree3_total_brLens'] = pTree.getPartLen(r0)[0]
                features['Subtree4_total_brLens'] = features['Subtree2_total_brLens'] - features[
                    'Subtree3_total_brLens']

                try:
                    features['Subtree1/3_brLens_ratio'] =  features['Subtree1_total_brLens']/features['Subtree3_total_brLens']
                except Exception as e:
                    features['Subtree1/3_brLens_ratio'] = -1
                ...
                features['Subtree1_var_brLens'], features['Subtree2_var_brLens'], _, br_list2 = pTree.getPartVarLen(y0)
                features['Subtree3_var_brLens'], _, br_list1, _ = pTree.getPartVarLen(r0)
                ...
                #     features['A_Prune_brLen'] = 0
                # try:
                #     features['A_Regraft_brLen'] = r0.br.len
                # except Exception as e:
                #     features['A_Regraft_brLen'] = 0
                # if features['A_Regraft_brLen'] ==0:
                #     features['A_Prune_regraft_brLen_ratio'] = -1
                # else:
                #     try:
                #         features['A_Prune_regraft_brLen_ratio'] = features['A_Prune_brLen']/features['A_Regraft_brLen']
                #     except Exception as e:
                #         features['A_Prune_regraft_brLen_ratio'] = -1
                #
                # # total = 0
                ...
                #                 # path += 1
                #                 # total += r0_.br.len
                #                 br_list.append(r0_.br.len)
                #     # path += 1
                #     y0_ = y0_.parent
                #     if y0_.parent != None:
                #         # path += 1
                #         # total += y0_.br.len
                #         br_list.append(y0_.br.len)
                #
                # br_list=numpy.array(br_list)
                # features['A_Prune2Regraft_total_leafNum'] = 0
                # features['A_Prune2Regraft_total_brLens'] = 0
                # features['A_Prune2Regraft_total_MaxbrLen'] = 0
                # features['A_Prune2Regraft_total_VarbrLen'] = 0
                ...
                # features['A_Subtree1_leafNum'] = len(list(y0.iterLeaves()))
                # features['A_Subtree2_leafNum'] = pTree.data.nTax - len(list(y0.iterLeaves()))
                # features['A_Subtree3_leafNum'] = len(list(r0.iterLeaves()))
                # features['A_Subtree4_leafNum'] = features['A_Subtree2_leafNum'] - len(list(r0.iterLeaves()))
                # features['A_Subtree1_total_brLens'], features['A_Subtree2_total_brLens'] = pTree.getPartLen(y0)
                # features['A_Subtree3_total_brLens'] = pTree.getPartLen(r0)[0]
                # features['A_Subtree4_total_brLens'] = features['A_Subtree2_total_brLens'] - features[
                #     'A_Subtree3_total_brLens']
                # try:
                #     features['A_Subtree1/3_brLens_ratio'] =  features['A_Subtree1_total_brLens']/features['A_Subtree3_total_brLens']
                # except Exception as e:
                #     features['A_Subtree1/3_brLens_ratio'] = -1
                #
                # features['A_Subtree1_max_brLen'], features['A_Subtree2_max_brLen'] = pTree.getPartMaxLen(y0)
                # features['A_Subtree3_max_brLen'], features['A_Subtree4_max_brLen'] = pTree.getPartMaxLen(r0, y0)
                #
                # features['A_Subtree1_var_brLens'], features['A_Subtree2_var_brLens'],_,br_list2 = pTree.getPartVarLen(y0)
                # features['A_Subtree3_var_brLens'],_,br_list1,_ = pTree.getPartVarLen(r0)
                #
                # unique_vals, counts = numpy.unique(br_list1, return_counts=True)
                # freq_dict = dict(zip(unique_vals, counts))
                # mask = numpy.ones(len(br_list2), dtype=bool)
                ...
                # feature extraction:
                features = {}
                features['feature_mode'] = 3
                features['Total_brLens'] = pTree.getLenMirror(c=0)
                features['Max_brLen'] = pTree.getMaxLen()
                features['Var_brLen'] = pTree.getVarLen()
                features['Prune_brLen'] = y0.br.len
                features['Regraft_brLen'] = r0.br.len
                features['Prune_regraft_brLen_ratio'] = y0.br.len/r0.br.len
                # total = 0
                # path = 0
                br_list=[]
                y0_ = y0
                ...
                br_list=numpy.array(br_list)
                features['Prune2Regraft_total_leafNum'] = len(list(y0_.iterLeaves()))
                features['Prune2Regraft_total_brLens'] = pTree.getPartLen(y0_)[0]
                features['Prune2Regraft_total_MaxbrLen'] = pTree.getPartMaxLen(y0_)[0]
                features['Prune2Regraft_total_VarbrLen'] = pTree.getPartVarLen(y0_)[0]

                features['Prune2Regraft_nodeNum'] = len(br_list)
                features['Prune2Regraft_brLen'] = numpy.sum(br_list)
                if features['Prune2Regraft_nodeNum']==0:
                    features['Prune2Regraft_Max_brLen']=0
                else:
                    features['Prune2Regraft_Max_brLen'] = numpy.max(br_list)
                features['Prune2Regraft_Var_brLen'] = numpy.var(br_list)
                ...
                # features['A_Prune_brLen'] = y0.br.len
                # features['A_Regraft_brLen'] = r0.br.len
                # features['A_Prune_regraft_brLen_ratio'] = y0.br.len/r0.br.len
                # # total = 0
                # # path = 0
                # br_list=[]
                # y0_ = y0
                ...
                #
                # br_list=numpy.array(br_list)
                # features['A_Prune2Regraft_total_leafNum'] = len(list(y0_.iterLeaves()))
                # features['A_Prune2Regraft_total_brLens'] = pTree.getPartLen(y0_)[0]
                # features['A_Prune2Regraft_total_MaxbrLen'] = pTree.getPartMaxLen(y0_)[0]
                # features['A_Prune2Regraft_total_VarbrLen'] = pTree.getPartVarLen(y0_)[0]
                #
                # features['A_Prune2Regraft_nodeNum'] = len(br_list)
                # features['A_Prune2Regraft_brLen'] = numpy.sum(br_list)
                # if features['A_Prune2Regraft_nodeNum']==0:
                #     features['A_Prune2Regraft_Max_brLen']=0
                # else:
                #     features['A_Prune2Regraft_Max_brLen'] = numpy.max(br_list)
                # features['A_Prune2Regraft_Var_brLen'] = numpy.var(br_list)
                #
                # features['A_Subtree1_leafNum'] = len(list(y0.iterLeaves()))
                # features['A_Subtree2_leafNum'] = pTree.data.nTax - len(list(y0.iterLeaves()))
                # features['A_Subtree3_leafNum'] = len(list(r0.iterLeaves()))
        ...
        if x0Uncon:
            ...
            if features is not None:
                features['Reshape_brLens'] = eA.len + eX.len
                features['Reshape_mul_brLens'] = eA.len * eX.len
        # eY.
        # if y0Uncon:
        #     u = random.random()
        #     lnMY = etbrLambda * (u - 0.5)
    ...
    def proposeNNI_Blaise(self, theProposal,extract_features=False):
        return self.proposeESPR_Blaise_FromETBR(theProposal, is_NNI=1, extract_features=extract_features)
